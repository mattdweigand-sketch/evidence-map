...
import { buildGeneratedEditOutput, type GeneratedEditProposal } from "./generated-edits.ts";
...
export interface GeneralFinalExport {
  ...
  generatedEditProposal?: GeneratedEditProposal;
  generatedEditProposalMarkdown?: string;
  editedOutputMarkdown?: string;
  blockers: string[];
...
export function buildGeneralFinalExport(input: {
  ...
  const formattedOutput =
    ready && input.outputMode === "generate" && input.generatedOutput && input.generatedClaims && input.sourceEvidence && input.sourceExclusions
      ...
      : undefined;
  const generatedEditOutput =
    ready && input.outputMode === "generate" && input.generatedOutput && input.generatedClaims && input.sourceEvidence && input.sourceExclusions
      ? buildGeneratedEditOutput({
          run: input.run,
          trustReport: input.trustReport,
          generatedOutput: input.generatedOutput,
          generatedClaims: input.generatedClaims,
  ...
  return {
    ...
    generatedEditProposal: generatedEditOutput?.proposal,
    generatedEditProposalMarkdown: generatedEditOutput?.proposalMarkdown,
    editedOutputMarkdown: generatedEditOutput?.editedMarkdown,
    blockers,
    acceptedRisks,
    unresolvedRisks
  };
...
function buildReadyManifest(input: {
  ...
  if (input.generatedOutput && input.evidenceMap && input.generatedClaims && input.sourceEvidence) {
    ...
    artifacts.generatedEditProposal = "04_export/generated-edit-proposal.json";
    artifacts.editedOutput = "04_export/edited-output.md";
  }
  ...
  return {
    ...
    notes: [
      "This manifest records local readiness for the generated Markdown or approved-artifact review path only.",
      ...
      input.generatedOutput
        ? "The general profile generated local Markdown claim receipts from verified claims, including formatted and edited deterministic derivatives."
...
Generated Markdown receipt: ${input.generatedOutput ? "`04_export/final-output.md`" : "not generated by the general profile"}
...
${input.ready ? "- Local ready manifest is available for the review packet/generation scope. No original file or native artifact was certified for external shipping." : renderList(input.blockers)}
...
Generated edit proposal: ${manifest.artifacts.generatedEditProposal ? `\`${manifest.artifacts.generatedEditProposal}\`` : "not present"}

Edited output: ${manifest.artifacts.editedOutput ? `\`${manifest.artifacts.editedOutput}\`` : "not present"}
