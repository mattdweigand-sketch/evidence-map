...
import type { OutputMode, Readiness, SourceConflict, TrustReport } from "../types.ts";

export async function evaluateTrust(
  store: EvidenceMapStore,
  runId: string,
  options: { sourceConflicts?: SourceConflict[]; outputMode?: OutputMode } = {}
): Promise<TrustReport> {
  const sources = await store.listSources(runId);
  const claims = options.outputMode === "generate" ? await store.listGeneratedClaims(runId) : await store.listClaims(runId);
  ...
  const needsReviewCount = findings.filter((finding) => finding.humanReviewRequired && finding.severity !== "polish").length;
